...
namespace Quad64.JSON
{
	public class SettingsFile
	{
		public static void SaveGlobalSettings(string profileName)
		{
			...
			string savePath = Path.Combine(".", "data", "profiles", profileName);
			Directory.CreateDirectory(savePath); // Create directory if it doesn't exist!
			File.WriteAllText(Path.Combine(savePath, "Settings.json"), s.ToString());
		}

		public static void LoadGlobalSettings(string profileName)
		{
			string fp = Path.Combine(".", "data", "profiles", profileName, "Settings.json");
			if (File.Exists(fp))
			{
